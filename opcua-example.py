...
# Conveyor nodes:
node_conveyor = opcua_client.get_node(f'ns={transport_ns_idx};s=Cell.Conveyor')
node_conveyor_moving = opcua_client.get_node(f'ns={transport_ns_idx};s=Cell.Conveyor.Moving') # Bool
node_conveyor_moving_left = opcua_client.get_node(f'ns={transport_ns_idx};s=Cell.Conveyor.MovingLeft') # Bool
node_conveyor_moving_right = opcua_client.get_node(f'ns={transport_ns_idx};s=Cell.Conveyor.MovingRight') # Bool
node_conveyor_moving_position = opcua_client.get_node(f'ns={transport_ns_idx};s=Cell.Conveyor.Position') # Double
node_conveyor_moving_speed = opcua_client.get_node(f'ns={transport_ns_idx};s=Cell.Conveyor.Speed') # Double
# General status nodes:
node_emergency_stop_ok = opcua_client.get_node(f'ns={workplace_ns_idx};s=Cell.EStopOK') # Bool
node_operator_safety_ok = opcua_client.get_node(f'ns={workplace_ns_idx};s=Cell.OperatorSafetyOK') # Bool
...
status, message = node_robot.call_method(f'{robots_ns_idx}:SetOverride', speed_override)
...
status, message = node_robot.call_method(f'{robots_ns_idx}:SetFrames', tool_id, base_id)
...
status, message = node_robot.call_method(f'{robots_ns_idx}:StartMoveToHome')
...
position.Status = 0
position.Turn = 0
position.ToolFrameID = 1
position.BaseFrameID = 0
status, message = node_robot.call_method(f'{robots_ns_idx}:StartMoveToPos', position)
...
status, message = node_robot.call_method(f'{robots_ns_idx}:GripperControl', activate)
...
status, message = node_conveyor.call_method(f'{transport_ns_idx}:ConveyorControl', go_left, go_right)
